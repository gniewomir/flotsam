...
describe("tabAllowsCloseAlarmScheduling", () => {
  ...
  it("rejects anchored tabs", () => {
    expect(tabEligibleToBeClosed(base, new Set<number>([base.id || 0]))).toBe(
      false,
    );
  });
  ...
  it("rejects pinned tabs", () => {
    expect(
      tabEligibleToBeClosed({ ...base, pinned: true }, new Set<number>()),
    ).toBe(false);
  });

  it("rejects grouped tabs", () => {
    expect(
      tabEligibleToBeClosed({ ...base, groupId: 1 }, new Set<number>()),
    ).toBe(false);
  });

  it("rejects audible tabs", () => {
    expect(
      tabEligibleToBeClosed({ ...base, audible: true }, new Set<number>()),
    ).toBe(false);
  });
  ...
  it("treats muted / non-playing as not audible (audible false)", () => {
    expect(
      tabEligibleToBeClosed({ ...base, audible: false }, new Set<number>()),
    ).toBe(true);
  });
});
